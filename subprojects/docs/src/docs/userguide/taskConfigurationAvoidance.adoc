...
[NOTE]
====
Migrating plugin code as follow will lock the runtime to Gradle 4.9 and above. Plugin authors should refer to the migration for backward compatibility.
====
...
For this reason, the Gradle team have created a shim plugin to help plugin authors that needs to support older version of Gradle (below 4.9).

In your plugin's build script add the following:
```
repositories {
    gradlePluginPortal()
}
...
dependencies {
    implementation "org.gradle:lazy-shim:latest.release"
}
```

In your plugin code, use `TaskContainerUtil#register(...)`, `TaskContainerUtil#configureEach(...)`, and `TaskContainerUtil#named(...)` instead of `TaskContainer#register(...)`, `TaskContainer#configureEach(...)` and `TaskContainer#named(...)`.
Those shim methods will return a `TaskProviderShim` that act in all ways as a `TaskProvider`.
The method `TaskProviderShim#get(), and `TaskProviderShim#configure(...)` are available as homologous to `TaskProvider#get()` and `TaskProvider#configure(...)`.

[NOTE]
====
Although the library allow for backward compability, it is still recommended to move compatibility forward to 4.9 and later sooner rather than later.
The shim library is only provided as stop gap for plugins with exceptional backward compatibility requirements.
====
